...
# data=pd.read_csv('./data_chung/tpl.csv')
...
# for i in tqdm(data['rxn'].values):
#     mapped_smi,reagent_se=new_smi_react(i)
#     data.loc[data['rxn']==i,'new_rxn']=mapped_smi
#     data.loc[data['rxn']==i,'reagent_separated']=reagent_se
# # data[['new_rxn','reagent_separated']]=df['rxn'].apply(pd.Series)
# data=data.dropna(subset=['new_rxn','reagent_separated'])
# data.to_csv('./data_chung/tpl_mapped.csv')




data=pd.read_csv('./data_chung/tpl_mapped.csv')
data=data.fillna('')
